...
export const FreeFormItem: React.FC<FreeFormItemProps> = ({ 
  ...
  const changeLayer = (action: 'up' | 'down' | 'front' | 'back') => {
    ...
    const maxZ = uniqueIndices[uniqueIndices.length - 1] || 1;
    const minZ = uniqueIndices[0] || 1;
      ...
      case 'image':
        return (
          <div className="w-full h-full relative group">
             <img src={item.content} className="w-full h-full object-cover pointer-events-none select-none" draggable={false} style={{ borderRadius: `${radius}px` }} />
             {item.caption && (
      ...
      case 'quote':
        return (
          <div className="w-full h-full p-6 flex flex-col justify-center items-center">
             ...
             <p className="italic leading-relaxed select-none w-full" style={textStyle}>"{item.content}"</p>
             {item.subContent && <p className="mt-4 text-[10px] font-bold uppercase tracking-widest opacity-60 border-t border-current/10 pt-2 select-none w-full" style={{ fontFamily }}>- {item.subContent}</p>}
          </div>
        );
      case 'affirmation':
        return (
          <div className="w-full h-full flex flex-col justify-center items-center p-6">
             <h3 className="font-black tracking-tighter uppercase leading-none select-none w-full" style={textStyle}>{item.content}</h3>
          </div>
        );
      case 'sticker':
        const StickerIcon = (LucideIcons as any)[item.content] || LucideIcons.HelpCircle;
        return (
          <div className="w-full h-full flex items-center justify-center p-2">
            <StickerIcon size="100%" strokeWidth={1.5} color={txtColor} />
          </div>
        );
      case 'goal_ref':
        return (
          <div className="w-full h-full p-6 bg-gradient-to-br from-gray-900/90 to-gray-800/90 text-white flex flex-col justify-between select-none" style={{ borderRadius: `${radius}px` }}>
             <div className="relative z-10">
                <div className="flex items-center gap-2 mb-2 opacity-70 text-[9px] font-black uppercase tracking-[0.3em]"><Target size={12} strokeWidth={3} /> Goal Active</div>
             ...
             <div className="mt-4 relative z-10">
                <div className="flex justify-between text-[10px] mb-2 opacity-90 font-mono font-black"><span className="uppercase tracking-widest">Mastery</span><span>{item.subContent}%</span></div>
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-white rounded-full transition-all duration-1000 shadow-[0_0_10px_white]" style={{ width: `${item.subContent}%` }} /></div>
             </div>
          </div>
        );
      default: return null;
  ...
  const toolbarStyle: React.CSSProperties = isMobile ? {
    position: 'fixed', bottom: 0, left: 0, right: 0, top: 'auto', transform: 'none', zIndex: 100, width: '100%', maxWidth: '100%'
  } : {
    top: '50%', left: 0, position: 'absolute', transform: `translateY(-50%) translateX(-100%) translateX(-24px) rotate(${-localRot}deg)`, transformOrigin: 'right center', pointerEvents: 'none'
  };
  ...
  return (
    <>
      ...
      >
          ...
          {isEditing && isSelected && (
             <>
               <div 
                 ...
                 className="absolute bottom-0 right-0 w-8 h-8 bg-white text-primary-600 rounded-tl-xl shadow-lg cursor-nwse-resize z-50 flex items-center justify-center border-t border-l border-gray-200"
               >
                  <Scaling size={14} />
               </div>
               ...
               <div 
                 ...
                 className="absolute -top-8 left-1/2 -translate-x-1/2 w-10 h-10 bg-white text-primary-600 rounded-full shadow-lg border border-gray-200 flex items-center justify-center cursor-grab active:cursor-grabbing hover:scale-110 transition-transform z-50"
               >
               ...
               <button 
                 ...
                 onTouchStart={(e) => { e.stopPropagation(); onDelete(item.id); }}
                 className="absolute -top-3 -right-3 w-8 h-8 bg-red-500 text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 transition-transform cursor-pointer z-50 border-2 border-white"
               >
        ...
        {isEditing && isSelected && (
           <div className={isMobile ? "fixed bottom-0 left-0 right-0 z-[100] pb-safe" : "z-[100]"} style={isMobile ? undefined : toolbarStyle}>
              <div 
                className={`bg-white dark:bg-gray-800 shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col gap-2 pointer-events-auto animate-in duration-200 ${isMobile ? 'w-full rounded-t-3xl p-4 slide-in-from-bottom-full border-b-0' : 'rounded-2xl p-1.5 min-w-[240px] slide-in-from-right-2 zoom-in-95'}`}
              ...
              >
                  <div className="flex items-center gap-1 bg-gray-50 dark:bg-gray-900/50 p-1 rounded-xl shrink-0 overflow-x-auto no-scrollbar">
                     <button onClick={() => setActiveTool(activeTool === 'style' ? 'none' : 'style')} className={`p-2 rounded-lg flex-1 flex justify-center ${activeTool === 'style' ? 'bg-white dark:bg-gray-700 text-primary-600 shadow-sm' : 'text-gray-500'}`}><Sliders size={18} /></button>
                     <button onClick={() => setActiveTool(activeTool === 'color' ? 'none' : 'color')} className={`p-2 rounded-lg flex-1 flex justify-center ${activeTool === 'color' ? 'bg-white dark:bg-gray-700 text-primary-600 shadow-sm' : 'text-gray-500'}`}><Palette size={18} /></button>
                     <button onClick={() => setActiveTool(activeTool === 'text' ? 'none' : 'text')} className={`p-2 rounded-lg flex-1 flex justify-center ${activeTool === 'text' ? 'bg-white dark:bg-gray-700 text-primary-600 shadow-sm' : 'text-gray-500'}`}><Type size={18} /></button>
                     <button onClick={() => setActiveTool(activeTool === 'layers' ? 'none' : 'layers')} className={`p-2 rounded-lg flex-1 flex justify-center ${activeTool === 'layers' ? 'bg-white dark:bg-gray-700 text-primary-600 shadow-sm' : 'text-gray-500'}`}><Layers size={18} /></button>
                     {isMobile && <button onClick={() => onSelect()} className="p-2 ml-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500"><X size={18} /></button>}
                  </div>
                    ...
                    {activeTool === 'style' && (
                       <div className="p-3 space-y-4">
                          <div>
                             ...
                             <input type="range" min="0" max="100" value={radius} onChange={(e) => updateStyle('borderRadius', parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-full appearance-none accent-primary-600" />
                          </div>
                          <div>
                             ...
                             <input type="range" min="0.1" max="1" step="0.1" value={opacity} onChange={(e) => updateStyle('opacity', parseFloat(e.target.value))} className="w-full h-2 bg-gray-200 rounded-full appearance-none accent-primary-600" />
                          </div>
                          <div className="flex gap-1 pt-1">
                             {['none', 'sm', 'lg', 'xl'].map(s => <button key={s} onClick={() => updateStyle('shadow', s)} className={`flex-1 py-2 text-[10px] font-bold uppercase rounded-lg border transition-all ${shadow === s ? 'bg-primary-50 border-primary-500 text-primary-600' : 'border-gray-200 text-gray-500'}`}>{s}</button>)}
                          </div>
                       </div>
                    )}
                    {activeTool === 'color' && (
                       <div className="p-3 space-y-4">
                          <div><p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Background</p><div className="flex flex-wrap gap-3">{COLORS.map(c => <button key={c} onClick={() => updateStyle('backgroundColor', c)} className={`w-6 h-6 rounded-full border border-gray-200 shadow-sm transition-transform ${bgColor === c ? 'ring-2 ring-primary-500 scale-110' : ''}`} style={{ background: c }} />)}</div></div>
                          <div className="h-px bg-gray-100 dark:bg-gray-700" />
                          <div><p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Text / Icon</p><div className="flex flex-wrap gap-3">{COLORS.map(c => <button key={c} onClick={() => updateStyle('textColor', c)} className={`w-6 h-6 rounded-full border border-gray-200 shadow-sm transition-transform ${txtColor === c ? 'ring-2 ring-primary-500 scale-110' : ''}`} style={{ background: c }} />)}</div></div>
                       </div>
                    )}
                    {activeTool === 'text' && (
                       <div className="p-3 space-y-4">
                          ...
                          <div><div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase mb-2"><span>Size</span><span>{fontSize}px</span></div><input type="range" min="12" max="150" value={fontSize} onChange={(e) => updateStyle('fontSize', parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-full appearance-none accent-primary-600" /></div>
                          <div><label className="text-[10px] font-bold text-gray-400 uppercase mb-2 block">Font Family</label><select value={fontFamily} onChange={(e) => updateStyle('fontFamily', e.target.value)} className="w-full text-sm p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 focus:outline-none focus:border-primary-500">{FONTS.map(f => <option key={f} value={f} style={{ fontFamily: f }}>{f}</option>)}</select></div>
                       </div>
                    )}
                    {activeTool === 'layers' && (
